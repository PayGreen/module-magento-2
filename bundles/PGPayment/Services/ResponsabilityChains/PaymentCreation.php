<?php
/**
 * 2014 - 2021 Watt Is It
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the MIT License X11
 * that is bundled with this package in the file LICENSE.md.
 * It is also available through the world-wide-web at this URL:
 * https://opensource.org/licenses/mit-license.php
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to contact@paygreen.fr so we can send you a copy immediately.
 *
 * @author    PayGreen <contact@paygreen.fr>
 * @copyright 2014 - 2021 Watt Is It
 * @license   https://opensource.org/licenses/mit-license.php MIT License X11
 * @version   2.1.0
 *
 */

/**
 * Class PGPaymentServicesResponsabilityChainsPaymentCreation
 * @package PGPayment\Services\ResponsabilityChains
 */
class PGPaymentServicesResponsabilityChainsPaymentCreation
{
    /** @var PGPaymentInterfacesPaymentCreationChainLink|null  */
    private $firstChainLinks = null;

    /** @var PGPaymentInterfacesPaymentCreationChainLink|null  */
    private $lastChainLinks = null;

    public function addChainLink(PGPaymentInterfacesPaymentCreationChainLink $chainLink)
    {
        if ($this->firstChainLinks === null) {
            $this->firstChainLinks = $chainLink;
            $this->lastChainLinks = $chainLink;
        } else {
            $this->lastChainLinks->setNext($chainLink);
            $this->lastChainLinks = $chainLink;
        }
    }

    public function buildPaymentCreationData(
        PGPaymentInterfacesEntitiesButtonInterface $button,
        PGShopInterfacesProvisionersPrePayment $prePaymentProvisionner
    ) {
        if ($this->firstChainLinks === null) {
            throw new Exception("Payment creation responsability chain is empty.");
        }

        $paymentProject = new PGPaymentComponentsPaymentProject($button, $prePaymentProvisionner);

        $this->firstChainLinks->run($paymentProject);

        return $paymentProject->toArray();
    }
}
