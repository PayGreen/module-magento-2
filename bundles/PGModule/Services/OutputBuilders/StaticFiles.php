 ...
 */
class PGModuleServicesOutputBuildersStaticFiles extends PGModuleFoundationsOutputBuilder
{
    /**
     * @inheritDoc
     */
    public function build(array $data = array())
    {
        ...
        if ($this->hasConfig('js')) {
            foreach ($this->getConfig('js') as $path) {
                $output->addResource(new PGServerComponentsResourcesScriptFileResource($path));
            }
        }

        if ($this->hasConfig('css')) {
            foreach ($this->getConfig('css') as $path) {
                $output->addResource(new PGServerComponentsResourcesStyleFileResource($path));
            }
        }
