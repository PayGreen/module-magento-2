 ...
 */
class FOPaymentServicesLinkersRetryPaymentValidation implements PGServerInterfacesLinkerInterface
{
    /** @var PGPaymentServicesHandlersPaymentCreationHandler */
    private $paymentCreationHandler;

    public function __construct(PGPaymentServicesHandlersPaymentCreationHandler $paymentCreationHandler)
    {
    ...
    public function buildUrl(array $data = array())
    {
        ...
        } elseif (!$data['task'] instanceof PGPaymentComponentsTasksPaymentValidation) {
            throw new Exception(
                "Building retry payment validation URL require PGPaymentComponentsTasksPaymentValidation entity."
            );
        }
