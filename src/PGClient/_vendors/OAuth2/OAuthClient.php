    ...
    */
    ...
    public function __construct($client_id, $client_secret, $client_auth = self::AUTH_TYPE_URI, $certificate_file = null)
    {
        if (!extension_loaded('curl')) {
            throw new OAuthException('The PHP exention curl must be installed to use this library.', OAuthException::CURL_NOT_FOUND);
        }
        ...
        if (!empty($this->certificate_file)  && !is_file($this->certificate_file)) {
            throw new OAuthInvalidArgumentException('The certificate file was not found', OAuthInvalidArgumentException::CERTIFICATE_NOT_FOUND);
        }
    }
    ...
    {
        require_once(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'GrantType' . DIRECTORY_SEPARATOR . 'AuthorizationCode.php');

        if (!$grant_type) {
            throw new OAuthInvalidArgumentException('The grant_type is mandatory.', OAuthInvalidArgumentException::INVALID_GRANT_TYPE);
        }

        $grantTypeClassName = $this->convertToCamelCase($grant_type);
        $grantTypeClass =  $grantTypeClassName;
        if (!class_exists($grantTypeClass)) {
            throw new OAuthInvalidArgumentException('Unknown grant type \'' . $grant_type . '\'', OAuthInvalidArgumentException::INVALID_GRANT_TYPE);
        }
        $grantTypeObject = new $grantTypeClass();
        $grantTypeObject->validateParameters($parameters);
        if (!defined($grantTypeClass . '::GRANT_TYPE')) {
            throw new OAuthException('Unknown constant GRANT_TYPE for class ' . $grantTypeClassName, OAuthException::GRANT_TYPE_ERROR);
        }
        ...
        switch ($this->client_auth) {
            ...
            case self::AUTH_TYPE_AUTHORIZATION_BASIC:
                ...
                break;
            default:
                throw new OAuthException('Unknown client auth type.', OAuthException::INVALID_CLIENT_AUTHENTICATION_TYPE);
                break;
        }
        ...
        if ($this->access_token) {
            switch ($this->access_token_type) {
                ...
                case self::ACCESS_TOKEN_MAC:
                    ...
                    break;
                default:
                    throw new OAuthException('Unknown access token type.', OAuthException::INVALID_ACCESS_TOKEN_TYPE);
                    break;
            }
        }
