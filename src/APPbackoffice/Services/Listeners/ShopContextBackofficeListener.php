...
class APPbackofficeServicesListenersShopContextBackofficeListener
{
    ...
    public function listen(PGServerComponentsActionEvent $event)
    {
        if ($this->shopHandler->isMultiShopActivated() && !$this->shopHandler->isShopContext()) {
            $this->notifier->add(PGFrameworkServicesNotifier::STATE_FAILURE, 'backoffice.errors.shop_context_needed');
        }
    }
}
