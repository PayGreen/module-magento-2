...
class Logout extends AbstractActionAdmin
{
    public function execute()
    {
        ...
        try {
            foreach (['public_key', 'private_key'] as $name) {
                $logger->debug("Undefine '$name' parameter.");
                $settings->set($name, '');
            }

            $this->addSuccessMessage('backoffice.actions.credentials.reset.result.success');
        } catch (Exception $exception) {
            $logger->alert("Error during reseting credentials : " . $exception->getMessage());

            $this->addErrorMessage('backoffice.actions.credentials.reset.result.failure');
        }
